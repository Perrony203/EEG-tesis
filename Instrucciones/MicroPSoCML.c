...
/******************************************** CONFIGURATION PROCESS ********************************************/
	...
	if(Readed_ID == Def_values[0]){
		...
		if(all_good){
			...
			printf("================================================== \r\n\n");
			/*Toggle leds for status indication*/
...
********************************************************************************/
			for (;;){
				...
				if(Interrupt_Occurred){
					...
					/* Format and store data and features */
					ProcessValues(valoresReales, chs2Read, CHS);


					// PRE-TRAINING PROCCES: AQUISITION, FILTERING, FEATURES AND COMMUNICATION
...
//					// POST-TRAINING PROCCES: CLASSIFICATION AND RESULT COMMUNICATION
//
//					/*Storing and organizing SVM characteristics*/
//					for (int i = 0; i < NCHANNELS; i++){
//						for (int j = 0; j < NCHARACS; j++){
//							svm_feats[j + 8*i + 4*feat_control] =characs[i][j];
//						}
//					}
//					feat_control ++;
...
//					/*Classification*/
//					if(feat_control >= 2){
//						classification = predict(svm_feats);
//						feat_control = 0;
//					}
