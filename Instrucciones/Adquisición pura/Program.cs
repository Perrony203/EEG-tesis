                        ...
                        using (TcpClient client = server.AcceptTcpClient()){
                            using (NetworkStream stream = client.GetStream()){
                                ...
                                while (true){
                                    ...
                                    if (valoresString.Length != 5) continue;
                                    ...
                                    for (int i = 0; i < valoresString.Length; i++){
                                        if (double.TryParse(valoresString[i], out double value)){
                                            valoresLeidos[i] = Math.Round(value, 6)*(5e-6)/3.3;
                                        }
                                        else{
                                            allgood = false;
                                        }
                                    ...
                                    if (allgood){
                                        string datos = $"{timestamp}; {valoresLeidos[0]}; {valoresLeidos[1]}; {valoresLeidos[2]}; {valoresLeidos[3]}; {valoresLeidos[4]}\n";
